            ...
            <div class="col-12 col-md-10 col-lg-8 col-xl-7">
                <div class="card shadow-lg border-0 rounded-4 p-4" style="background: rgba(255,255,255,0.98);">
                    <form method="POST" action="{{ route('admin.testimonials.update', $testimonial->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="row g-3">
                            <div class="col-12">
                                <label class="form-label fw-semibold" style="color:#2C3E50;">Name <span class="text-danger">*</span></label>
                                <input type="text" name="name" class="form-control rounded-3 border-2" style="border-color: #4fc3f7;" required value="{{ old('name', $testimonial->name) }}">
                            </div>
                            <div class="col-12">
                                <label class="form-label fw-semibold" style="color:#2C3E50;">Photo</label>
                                @if($testimonial->photo)
                                    <div class="mb-2">
                                        <img src="{{ asset('storage/images/StudentPictures/' . $testimonial->photo) }}" alt="{{ $testimonial->name }}" class="rounded-circle border-3 border-dark mb-2" style="width: 90px; height: 90px; object-fit: cover;">
                                    </div>
                                @endif
                                <input type="file" name="photo" class="form-control rounded-3 border-2 w-100" style="border-color: #4fc3f7;" accept="image/*">
                                <small class="text-muted">Leave blank to keep current photo.</small>
                            </div>
                            <div class="col-12 col-md-6">
                                <label class="form-label fw-semibold" style="color:#2C3E50;">School</label>
                                <input type="text" name="school" class="form-control rounded-3 border-2" style="border-color: #4fc3f7;" value="{{ old('school', $testimonial->school) }}">
                            </div>
                            <div class="col-12 col-md-6">
                                <label class="form-label fw-semibold" style="color:#2C3E50;">City</label>
                                <input type="text" name="city" class="form-control rounded-3 border-2" style="border-color: #4fc3f7;" value="{{ old('city', $testimonial->city) }}">
                            </div>
                            <div class="col-12">
                                <label class="form-label fw-semibold" style="color:#2C3E50;">Testimonial Text <span class="text-danger">*</span></label>
                                <textarea name="text" class="form-control rounded-3 border-2" rows="4" style="border-color: #4fc3f7;" required>{{ old('text', $testimonial->text) }}</textarea>
                            </div>
                            <div class="col-12 col-md-6 mt-2">
                                <div class="d-flex align-items-center gap-4">
                                    <div class="form-check d-flex align-items-center" style="min-height: 48px;">
                                        <input type="checkbox" name="isFeatured" class="form-check-input mt-0" id="isFeatured" {{ old('isFeatured', $testimonial->isFeatured) ? 'checked' : '' }}>
                                        <label class="form-check-label fw-semibold ms-2" for="isFeatured">Featured</label>
                                    </div>
                                    <div class="form-check d-flex align-items-center" style="min-height: 48px;">
                                        <input type="checkbox" name="active" class="form-check-input mt-0" id="active" {{ old('active', $testimonial->active) ? 'checked' : '' }}>
                        ...
                        <div class="text-center mt-4">
                            ...
                            <a href="{{ route('admin.testimonials.index') }}" class="btn btn-lg px-5 py-3 fw-semibold rounded-3 shadow ms-2"
