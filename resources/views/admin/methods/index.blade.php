...
<section style="overflow: hidden; position: relative;">
            ...
            <div class="row align-items-center justify-content-center">
                <div class="col-12 col-lg-6 text-white pt-5 mt-5 text-lg-start text-center ps-lg-5" style="margin-top: 6rem !important;">
                    ...
                    <p class="fs-5 mb-4" style="font-size: 1.1rem; text-shadow: 1px 1px 2px rgba(0,0,0,0.12); max-width: 520px;">
                        Create, edit, and organize all learning methods at COTHA.
                    </p>
                </div>
    ...
    {{-- Search and Add New Method Row --}}
    <div class="container py-5 position-relative" style="z-index: 2;">
        <div class="row align-items-center mb-4">
            <div class="col-12 col-md-7">
                <form method="GET" action="{{ route('admin.methods.index') }}" class="d-flex gap-2 align-items-center">
                    <input type="text"
                           name="search"
            ...
            <div class="col-12 col-md-5 text-end mt-3 mt-md-0">
                <a href="{{ route('admin.methods.create') }}"
            ...
            <table class="table table-bordered table-striped align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('admin.methods.edit', $method) }}" class="btn btn-sm btn-warning rounded-3">
                                <i class="bi bi-pencil"></i> Edit
                            </a>
                            <form action="{{ route('admin.methods.destroy', $method) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-sm btn-danger rounded-3" onclick="return confirm('Delete this method?')">
        ...
        @if ($methods->hasPages())
            <div class="text-center mt-5">
                <p class="text-muted mb-3">
                    Page {{ $methods->currentPage() }} of {{ $methods->lastPage() }}
                </p>
                <nav aria-label="Page navigation">
                    <ul class="pagination justify-content-center" style="gap: 8px;">
                        ...
                        @else
                            <li class="page-item">
                                <a class="page-link border-0 shadow-sm"
                                   ...
                                   href="{{ $methods->previousPageUrl() }}" rel="prev">
                                    <i class="bi bi-chevron-left"></i>
                                </a>
                            </li>
                        @endif
                        ...
                        {{-- Pagination Elements --}}
                        @php
                            $current = $methods->currentPage();
                            $last = $methods->lastPage();
                            $window = 2;
                            $pages = [];
                            $pages[] = 1;
                        ...
                        @foreach ($display as $item)
                            ...
                            @else
                                @php $url = $methods->url($item); @endphp
                                @if ($item == $current)
                        ...
                        @if ($methods->hasMorePages())
                            <li class="page-item">
                                <a class="page-link border-0 shadow-sm"
                                   style="background: #FF85A2; color: #fff; border-radius: 12px;"
                                   href="{{ $methods->nextPageUrl() }}" rel="next">
